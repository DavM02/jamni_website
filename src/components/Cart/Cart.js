...
export default function Cart() {
    ...
    const randomPage = useMemo(() => Math.floor(Math.random() * Math.max(1, Math.floor(dataLength / 18))), [dataLength])

    const { data, error, isLoading, } = useSWRImmutable(
        ['decor', randomPage, 13], loadData 
        ...
        <div id='cart' className='column s-between'>
            <div className='cart-content'>
                ...
                <ul className='cart-items column gap-40'>
                    ...
                    {
                        products && <AnimatePresence>
                            {products.map((el, i) => {
                                return <SmoothAppearance
                                    onClick={() => navigate(`catalog/${el.catalog}/${el.name.toLowerCase()}?id=${el.id}`)}
                                    Tag={motion.li}
                                    key={el.id}
                                    layout
                                        ...
                                        <div className='column gap-20 s-between'>
                                            <div>
                                                ...
                                                <span className="xsmall-text text-main text-black">
                                                    {(el.price * el.quantity).toLocaleString("ru-RU")} руб
                                                </span>
                                            </div>
                                            <div className='cart-item-quantity grid-3 xsmall-text text-main'>
                                                <div className='center-gr' onClick={(e) => {
                                                    e.stopPropagation();
                                                    el.quantity > 1 ? decreaseQuantity(el.id) : removeProduct(el.id)
                                                }}><span>-</span></div>
                                                ...
                                                <div className='center-gr' onClick={(e) => { e.stopPropagation(); increaseQuantity(el.id) }}><span>+</span></div>
                                            </div>
                                        </div>
                                    </div>
                                    <div className='remove-item center-gr'
                                        onClick={(e) => { e.stopPropagation(); removeProduct(el.id) }}
                                    >
            ...
            <div>
                <div className='show-recommendations'>
                    <div className='row center-y gap-5' onClick={() => setShowRecs((prev) => !prev)}>
                        <span className='xsmall-text text-main text-black up-case'>
                            Рекомендации для вас
                        </span>
                        <img style={{ transform: showRecs ? 'rotate(180deg)' : 'none' }} src={arrow} alt='arrow' />
                    </div>
                    <ul className='recommendations' style={{ height: showRecs ? '160px' : '0px' }}>
                        {data && dataLength && <InfiniteSlider sliderData={data.map((el) => { return { ...el, catalog: 'decor' } })} wrapper={Slide} />}
                    </ul>
                </div>
