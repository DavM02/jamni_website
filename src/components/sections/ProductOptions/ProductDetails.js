    ...
    return (
        <div className='product-details'>
            <div className='row gap-10 xxxsmall-text text-main text-gray up-case'>
                ...
                <span>/</span>
                <Link to={`/catalog/decor?collection=${catalog}`}>
                    {catalog}
                </Link>
            ...
            <div className="column gap-20">
                {
                    dimensions && dimensions.map((el, i) => {
                        ...
                        if (key === "ширина") {
                            ...
                            const heightDimension = dimensions.find(dim => Object.keys(dim)[0] === "высота");
                            const height = heightDimension?.высота || [];

                            return (
                                <div key={i}>
                                    <span className="option-key xsmall-text text-main text-black">Размер</span>
                                    <div className="options row wrap">
                                        {values.map((el, i) => (
                                            <Radio
                                                key={i}
                                                text={`${el.value}x${height[i]?.value || ''} ${el.unit}`}
                                                name="sizes"
                                            />
                                        ))}
                                    </div>
                            ...
                            return (
                                <div key={i}>
                                    <span className="option-key xsmall-text text-main text-black">{key}</span>
                                    <div className="options row wrap">
                                        {values.map((el, i) => (
                                            <Radio
                                                key={i}
