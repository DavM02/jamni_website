...
export default function AppLoading({ setRenderApp }) {
    ...
    return (
        ReactDOM.createPortal(<div id='app-loading' style={{ clipPath: reveal ? 'inset(0 0 100% 0)' : 'inset(0)' }}>
            <div className='app-loading-images'
                onTransitionEnd={(e) => {
                    setRenderApp(true)
                }}
                style={{ clipPath: reveal ? 'inset(0 0 100% 0)' : 'inset(0)' }}>
                {images.map((el, i) => {
                    return <div key={i}
                        id={i}
                        onTransitionEnd={(e) => {
                            e.stopPropagation()
                            if (e.currentTarget.id == 9) { setReveal(true) }
                        }}
                        style={{
                            clipPath: loaded ? 'inset(0)' : 'inset(0 100% 0 0)',
