    ...
    function handleFilter(mutate) {
        ...
        mutate((data) => {
            ...
            return data.map((el) => {
                ...
                const matchPrice = (max ? el.price <= max : true) && (min ? el.price >= min : true);
                const matchColors = selectedColors.length === 0 || el.colors.some((color) => selectedColors.includes(color));

                return { ...el, isShown: matchQuery && matchDescription && matchPrice && matchColors };
            });
        }, false);
    }
